...
/**
 * Controleur permettant de renvoyer la liste des Karazampifidianana suivant le début de libellé saisi comme filtre
 * @author Rija
 * @version 1.0
 */
public class AutoCompletionKarazamPifidiananaController implements Controller{
	...
	private Map<String, String> getParameterValue(HttpServletRequest request){
		...
		while(parameterNames.hasMoreElements()){
			...
			if (current != null){
				if (current.startsWith("karazamPifidianana"))
	...
	public ModelAndView handleRequestWithoutPageRenderer(HttpServletRequest request, HttpServletResponse resp) throws Exception {
		...
		List<ILabelable> karazamPifidiananas = fFjkmAnkadifotsyService.getKarazamPifidianana(parameters); 
		StringBuffer data = new StringBuffer("<?xml version=\"1.0\" encoding=\"ISO_8859-1\" ?>");
		data.append("<ajax-response>");
		data.append("<response>");
		data.append("<list>");
		for (ILabelable karazamPifidianana : karazamPifidiananas){
			data.append("<item>");
			data.append("<name><![CDATA["+karazamPifidianana.getLabel()+"]]></name>");
			data.append("<value><![CDATA["+karazamPifidianana.getId()+"]]></value>");
			data.append("</item>");
		}
		data.append("</list>");
		data.append("</response>");
	...
	public ModelAndView handleRequest(HttpServletRequest request, HttpServletResponse resp) throws Exception {
		...
		return new ModelAndView("labelableAjaxResponse", "data", karazamPifidiananas);
	}
}
